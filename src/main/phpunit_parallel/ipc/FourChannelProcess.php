...
/**
 * Taken from \React\ChildProcess\Process and extended to allow an additional
 * output channel from the process. ($this->comm)
 *
 * TODO: Submit a PR upstream with the ability to add pipes.
 */
class FourChannelProcess extends EventEmitter
{
    public $stdin;
    public $stdout;
    public $stderr;
    ...
    public function start(LoopInterface $loop, $interval = 0.1)
    {
        ...
        $fdSpec = array(
            ...
            array('pipe', 'w'), // stdout
            array('pipe', 'w'), // stderr
            array('pipe', 'w'), // comm pipe
        );

        // Read exit code through fourth pipe to work around --enable-sigchild
        if ($this->isSigchildEnabled() && $this->enhanceSigchildCompatibility) {
            $fdSpec[] = array('pipe', 'w');
            $cmd = sprintf('(%s) 4>/dev/null; code=$?; echo $code >&3; exit $code', $cmd);
        }
        ...
        $this->comm = new Stream($this->pipes[3], $loop);
